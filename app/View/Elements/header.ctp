...
<? if ($this->User->loggedIn()): ?>
     <a href="/geo/init/?location=<?= urlencode('/menus/add_restaurant/') ?>"><?= $this->Html->image('/img/icon-camera-taken.png', array('class'=>'menu-plus')) ?><?//= UHelper::pictAddBox('23pt') ?></a>
<? else: ?>
     <a href="/users/login/?location=<?= urlencode('/menus/add_restaurant/') ?>"><?= $this->Html->image('/img/icon-camera-taken.png', array('class'=>'menu-plus')) ?><?//= UHelper::pictAddBox('23pt') ?></a>
<? endif; ?>
   </div>
